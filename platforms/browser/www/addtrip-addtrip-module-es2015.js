...
let AddtripPage = class AddtripPage {
    ...
    populateCountriesArray() {
        this.travelAppService.fetchAllCountries().then((countries) => {
    ...
    uploadTrip() {
        this.loadingCtrl.create({ message: "Uploading your trip" }).then((res) => {
            res.present();
            this.travelAppService.addTrip(this.useruid, this.aboutTrip, this.city, this.parseDate(this.fromDate), this.parseDate(this.toDate), this.eventTitle, this.province, this.base64TripPhoto).then(() => {
                res.dismiss();
                this.aboutTrip = "";
                this.city = "";
                this.fromDate = "";
