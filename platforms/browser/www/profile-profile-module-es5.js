...
var ProfilePage = /** @class */ (function () {
    function ProfilePage(toastCtrl, camera, actionSheetCtrl, alertController, events, router, geolocation, loadingCtrl, travelAppService) {
        var _this = this;
        ...
        this.showTrips = true;
        this.showMaps = false;
        this.showTripsText = false;
        this.showGalleryText = false;
        this.galleryimages = [];
        this.trips = [];
        // this.useruid = "aLEkqIdM23eJosktD30UbyHZerf1";
        ...
        this.loadingCtrl.create({ message: "Fetching your profile details" }).then(function (res) {
            ...
            firebase__WEBPACK_IMPORTED_MODULE_4__["database"]().ref('/users').child(_this.useruid).once('value', function (snapshot) {
                ...
                if (snapshot.val().coverPhoto == undefined || snapshot.val().coverPhoto == "") {
                    _this.base64CoverPhoto = '../../assets/backg2.png';
                }
                else {
                            ...
                            buttons: [
                                {
                                    ...
                                    handler: function (alertData) {
                                        _this.loadingCtrl.create({ message: "Please wait" }).then(function (res) {
                                            res.present();
                                            _this.travelAppService.updateUserName(alertData.name).then(function () {
                                                _this.name = alertData.name;
                                                res.dismiss();
                                                _this.presentToast("Name updated successfully");
                                            }).catch(function (err) {
                                                res.dismiss();
                                                _this.presentToast("Name not updated" + err);
                                            });
                                        });
    ...
    ProfilePage.prototype.showTripDetails = function (i) {
    ...
    ProfilePage.prototype.checkTripsArray = function () {
        if (this.trips == [] || this.trips == undefined) {
            this.showTripsText = true;
        }
    };
    ProfilePage.prototype.checkGalleryArray = function () {
